...
function calculaIMC(pesoEmLibras, alturaEmCm){
  return (pesoEmLibras * 0.4536 / ((alturaEmCm / 100) * (alturaEmCm / 100)));
}
//-----------------/Funções Auxiliares

//----------------------01
function obterDoadores() {
  var doadores = goldSaints
                .filter(function(e){return e.tipoSanguineo === 'O'});
  return doadores;
}
//----------------------02
...
function obterAlturaMedia() {
  ...
  return arredondar((soma/goldSaints.length)/100);
}
//----------------------05
function obterAlturaMediana() {
  ...
  return arredondar(((menor + maior) / 2)/100);
}
//----------------------06
//----------------A
function obterPesoMedio(cavaleiros) {
  ...
  soma = soma * 0.4536;
  return arredondar(soma/comPeso);
}
//----------------B
...
//----------------------07
function obterIMC(){
  var IMCs = goldSaints.filter(function(e){
    return typeof e.pesoLb !== 'undefined';
  }).map(function(e){
    return arredondar(calculaIMC(e.pesoLb, e.alturaCm));
  });
  return IMCs;
}
//----------------------08
