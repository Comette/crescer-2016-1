...
$(function () {
  ...
  //------------------------ADD CAMPO GOLPE
  var $btnAddGolpe = $('#btnAddGolpe');
  var nomeGolpes = [];
  var qtdGolpes = 0;
  $btnAddGolpe.click(function () {
    nomeGolpes.push('g' + qtdGolpes);
    $('#formGolpes').append(
      $('<label>').attr('for', nomeGolpes[qtdGolpes]).text('Golpe ' + (qtdGolpes + 1))
    ).append(
      $('<input>').attr('id', nomeGolpes[qtdGolpes]).attr('name', nomeGolpes[qtdGolpes]).attr('type', 'text').attr('placeholder', 'Ex: Cólera do Dragão')
      );
    qtdGolpes++;
  })
  //------------------------ADD CAMPO IMAGEM
  var $btnAddImagem = $('#btnAddImagem');
  var nomeImagens = [];
  var nomeCheckbox = [];
  var qtdImagens = 0;
  $btnAddImagem.click(function () {
    nomeImagens.push('g' + qtdImagens);
    nomeCheckbox.push('cb' + qtdImagens);
    $('#formImagens').append(
      $('<label>').attr('for', nomeImagens[qtdImagens]).text('URL da Imagem ' + (qtdImagens + 1))
    ).append(
      $('<input>').attr('id', nomeImagens[qtdImagens]).attr('name', nomeImagens[qtdImagens]).attr('type', 'text').attr('placeholder', 'Ex: bit.ly/shiryu.png')
      ).append(
      $('<input>').attr('name', nomeCheckbox[qtdImagens]).attr('type', 'checkbox').attr('value', 'sim')
      ).append(
      $('<label>').attr('for', nomeCheckbox[qtdImagens]).text('É Thumbnail?')
      )
    qtdImagens++;
  })
  //-----------------------------PRINTA CAVALEIROS
  for (var i = 0; i <= Math.floor((goldSaints.length - 1) / 2); i++) {
    $('#div-esquerda .cavaleiros').append($('<li>').append($('<img>').attr('src', goldSaints[i].imagens[0].url).attr('alt', goldSaints[i].nome)).append('<p>'));
  }
  for (var i = Math.ceil((goldSaints.length - 1) / 2); i < goldSaints.length; i++) {
    $('#div-direita .cavaleiros').append($('<li>').append($('<img>').attr('src', goldSaints[i].imagens[0].url).attr('alt', goldSaints[i].nome)).append('<p>'));
  }
  //-----------------------------EXIBE DESCRIÇÃO
  $(".cavaleiros li").hover(function () {
    //----Define cavaleiro
    var self = this;
    var cavaleiros = goldSaints.filter(function (e) {
      return e.nome === $(self).children().attr('alt');
    });
    var cavaleiro = cavaleiros[0];
    //----Monta Descrição
    var $descricao = $('<p>Nome: ' + cavaleiro.nome + '<br>' + 'Data de Nascimento: ' + cavaleiro.dataNascimento + '<br>' + 'Altura: ' + (cavaleiro.alturaCm / 100) + '<br>' + 'Peso: ' + (cavaleiro.pesoLb * 0.4536) + '<br>' + 'Signo: ' + cavaleiro.signo + '<br>' + 'Tipo Sanguíneo: ' + cavaleiro.tipoSanguineo + '<br>' + 'Local de Nascimento: ' + cavaleiro.localNascimento + '<br>' + 'Locas de Treinamento: ' + cavaleiro.localTreinamento + '<br>' + 'Golpes: ' + cavaleiro.golpes + '</p>');
    //----Exibe Descrição
    $(this).addClass('mostrandoLi');
    $(this).children().next().html($descricao);
    $(this).children().next().show();
  }, function () {
    //----Remove Descrição
    $(this).removeClass('mostrandoLi');
    $(this).children().next().hide();
  });
  //-----------------------------PRINTA CAVALEIRO NOVO
  function renderizarCavaleiroNaTela(cavaleiro) {
    if (goldSaints.length % 2 === 0) {
      $('#div-esquerda .cavaleiros').append($('<li>').append($('<img>').attr('src', cavaleiro.imagens[0].url).attr('alt', cavaleiro.nome)).append('<p>'));
    } else {
      $('#div-direita .cavaleiros').append($('<li>').append($('<img>').attr('src', cavaleiro.imagens[0].url).attr('alt', cavaleiro.nome)).append('<p>'));
    }
  };
  //-----------------------------CONVERTE FORM PRA CAVALEIRO
  function converterFormParaCavaleiro($form) {
    var formData = new FormData($form[0]);
    var checkboxMarcadas = [];
    var urls = [];
    var golpesAdicionados = [];
    for (var i = 0; i < qtdImagens; i++) {
      checkboxMarcadas.push($('#formImagens').find('input[name=' + nomeCheckbox[i] + ']').is(':checked'));
      urls.push($('#formImagens').find('input[name=' + nomeImagens[i] + ']').val());
    }
    for (var i = 0; i < qtdGolpes; i++) {
      golpesAdicionados.push($('#formGolpes').find('input[name=' + nomeGolpes[i] + ']').val());
    }
    var cavaleiroPronto = {
